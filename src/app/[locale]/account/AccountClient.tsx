...
export default function AccountClient() {
  ...
  async function loadData() {
    ...
    try {
      ...
      if (json.ok) {
        setData({ email: json.email, license: json.license, flags: json.flags });
      }
    } catch (error) {
    ...
    <main className="mx-auto max-w-4xl px-4 py-12">
      ...
      {!orderId ? (
        <div className="glass-card p-8">
          ...
          <p className="text-gray-300 mb-4">Введите Order ID из письма или страницы успешной оплаты</p>
          <form
            onSubmit={(e) => {
              e.preventDefault();
              window.location.href = `/ru/account?order_id=${email}`;
            }}
        ...
        <div className="space-y-6">
          ...
          <div className="glass-card p-8">
            ...
            <div className="space-y-3">
              <p>
                <span className="text-gray-400">Email:</span> <span className="font-medium">{data.email}</span>
              </p>
              <p>
              ...
              {data.license?.seats && (
                <p>
                  <span className="text-gray-400">Мест:</span> <span className="font-medium">{data.license.seats}</span>
                </p>
              )}
            </div>
          </div>
