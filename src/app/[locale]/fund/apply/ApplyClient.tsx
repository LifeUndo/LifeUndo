...
export default function ApplyClient({ locale }: { locale: 'ru' | 'en' }) {
          ...
          <div className="glass-card p-8">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                >
                  <option value="" disabled className="text-gray-500">
                    {isEn ? 'Choose a category' : 'Выберите категорию'}
                  </option>
                  <option value="nko" className="text-white bg-gray-800">
                    {isEn
                      ? 'NGOs — civil society initiatives'
                      : 'НКО — инициативы гражданского общества'}
                  </option>
                  <option value="medicine" className="text-white bg-gray-800">
                    {isEn
                      ? 'Medicine — education for doctors/patients'
                      : 'Медицина — образовательные проекты для медиков/пациентов'}
                  </option>
                  <option value="education" className="text-white bg-gray-800">
                    {isEn
                      ? 'Education — school/university clubs'
                      : 'Образование — школы/вузовские кружки'}
                  </option>
                  <option value="media" className="text-white bg-gray-800">
                    {isEn
                      ? 'Media — local media and newsrooms'
                      : 'СМИ — локальные медиа и редакции'}
                  </option>
                  <option value="individuals" className="text-white bg-gray-800">
                    {isEn
                      ? 'Individuals — students, teachers, parents'
                      : 'Частные лица — студенты, преподаватели, родители'}
                  </option>
                </select>
                ...
                {formData.category && (
                  <div className="mt-3 p-4 bg-blue-500/10 border border-blue-500/20 rounded-lg">
                    <p className="text-sm text-gray-300">
                      {formData.category === 'nko' &&
                        (isEn
                          ? 'Digital literacy, privacy, safe data practices.'
                          : 'Цифровая грамотность, приватность, обучение безопасной работе с данными.')}
                      {formData.category === 'medicine' &&
                        (isEn
                          ? 'Education for doctors/patients on personal data protection and reducing communication errors.'
                          : 'Образовательные проекты для медиков/пациентов о защите персональных данных и снижении ошибок коммуникаций.')}
                      {formData.category === 'education' &&
                        (isEn
                          ? 'Grants for digital hygiene clubs, school newspapers, student communities.'
                          : 'Гранты на кружки по цифровой гигиене, школьные газеты, студэнсообщества.')}
                      {formData.category === 'media' &&
                        (isEn
                          ? 'Tools and training for safe publishing and fact-checking.'
                          : 'Инструменты и тренинги по безопасной публикации и проверке материалов.')}
                      {formData.category === 'individuals' &&
                        (isEn
                          ? 'Microgrants for software/education if the project brings public benefit.'
                          : 'Микрогранты на софт/обучение, если проект несёт общественную пользу.')}
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="flex items-start space-x-3">
                ...
                <label htmlFor="consent" className="text-sm text-gray-300">
                  {isEn
                    ? 'I consent to personal data processing and receiving notifications about the application status *'
            ...
            <div className="mt-8 p-4 bg-blue-500/10 border border-blue-500/20 rounded-lg">
              <h3 className="font-semibold text-blue-300 mb-2">
                {isEn ? 'About the fund' : 'Информация о фонде'}
              </h3>
              <p className="text-sm text-gray-300">
                {isEn
