  ...
  return (
    <main className="max-w-4xl mx-auto px-4 py-10 space-y-10">
      ...
      <section>
        ...
        <pre className="mt-3 bg-gray-900 p-3 rounded-lg overflow-auto">{`Authorization: Bearer <api_key>`}</pre>
      ...
      <section>
        ...
        <div className="space-y-6">
          <div className="bg-gray-800/50 p-4 rounded-xl">
            ...
            <pre className="mt-2 bg-gray-900 p-3 rounded-lg overflow-auto">{`GET /api/healthz -> 200 OK`}</pre>
          </div>

          <div className="bg-gray-800/50 p-4 rounded-xl">
            <h3 className="font-semibold">{t.apiPage.createTitle}</h3>
            <div className="text-sm text-gray-400 mt-2">{t.apiPage.requestBody}</div>
            <pre className="mt-2 bg-gray-900 p-3 rounded-lg overflow-auto">{`POST /api/payments/freekassa/create
...

{
  "productId": "pro_monthly | vip_lifetime | team_5_monthly | starter_6m"
            ...
            <pre className="mt-2 bg-gray-900 p-3 rounded-lg overflow-auto">{`200 OK
{
  "ok": true,
      ...
      <section>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div>
            ...
            <pre className="mt-2 bg-gray-900 p-3 rounded-lg overflow-auto">{`curl -X POST ${base}/api/payments/freekassa/create \\\n -H "Content-Type: application/json" \\\n -d '{"productId":"pro_monthly"}'`}</pre>
          </div>
          <div>
            <div className="text-sm text-gray-400">{t.apiPage.powershell}</div>
            <pre className="mt-2 bg-gray-900 p-3 rounded-lg overflow-auto">{`$body = @{ productId = "pro_monthly" } | ConvertTo-Json
Invoke-RestMethod -Method POST "${base}/api/payments/freekassa/create" -ContentType "application/json" -Body $body`}</pre>
          </div>
        </div>
      </section>
      ...
      <section>
        ...
        <Link href="/api/openapi.yaml" className="underline text-blue-400">
          openapi.yaml
        </Link>
      </section>
    </main>
