...
export default function DevelopersPage({ params }: { params: { locale: string } }) {
  ...
  const t = {
    ...
    payments: isEN ? 'Payments (trial start and status)' : 'Платежи (старт триала и статус)',
    createDesc: isEN ? 'Create payment or start trial (returns pay_url)' : 'Создать платёж/запустить триал (возвращает pay_url)',
