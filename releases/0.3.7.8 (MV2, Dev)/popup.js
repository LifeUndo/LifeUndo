...
// подставляем реальную версию из manifest, чтобы бейдж не отставал
try {
...
function renderList(container, items, type){
  ...
  for(const it of items){
    ...
    if(type==='text'||type==='clip'){
      const b=document.createElement('button'); b.className='copy'; b.textContent=(lang==='ru'?'Скопировать':'Copy');
...
applyI18n(); refresh();
setInterval(async ()=>{ try{ const st = await browser.runtime.sendMessage({type:'pull-state'}); if (!st.vip && !st.pro) statusChip.textContent = fmtTrial(st.trialLeftMs||0); }catch{} }, 60_000);
