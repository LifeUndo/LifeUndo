...
function addMoreToggle(li, txtEl){
  const btn=document.createElement('button');
  btn.className='more';
  btn.textContent=i18n[lang].more;
  btn.onclick=()=>{
    const expanded = li.classList.toggle('expanded');
    btn.textContent = expanded ? i18n[lang].less : i18n[lang].more;
  };
  return btn;
}
...
function renderList(container, items, type){
  ...
  for(const it of items){
    ...
    // «Показать ещё» только для длинного текста
    if ((it.val||'').length > 400) act.appendChild(addMoreToggle(li, txt));
...
/* Modal — «железное» открытие/закрытие */
