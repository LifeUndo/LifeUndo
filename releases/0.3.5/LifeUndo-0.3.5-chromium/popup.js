...
async function refreshVipUi(){
  const { lu_plan, license } = await api.storage.local.get(['lu_plan', 'license']);
  const isVip = lu_plan === 'vip' || !!(license && license.plan==='vip' && (!license.expiry || new Date(license.expiry)>=new Date()));
...
api.storage.onChanged.addListener((changes, area) => {
  ...
  if (changes.lu_plan || changes.license) refreshVipUi();
});
