...
const i18n={
  en:{text_title:"Latest text inputs",text_empty_title:"Nothing yet",text_empty_sub:"Type something on any regular page and refresh — entries will appear here.",
      ...
      status_free:"Free · Trial: {d}d {h}h"},
  ru:{text_title:"Недавние вводы текста",text_empty_title:"Пока пусто",text_empty_sub:"Наберите текст на обычной странице и обновите — записи появятся здесь.",
      ...
      status_free:"Бесплатная · Триал: {d}д {h}ч"}
};
let lang='ru';
...
function setListVisibility(count, listEl, emptyEl, clearBtn){
  if (count>0){ listEl.hidden=false; emptyEl.style.display='block'; emptyEl.style.display='none'; if(clearBtn) clearBtn.disabled=false; }
...
function fmtTrial(ms){
  ...
  const tpl=i18n[lang].status_free;
  return tpl.replace('{d}',d).replace('{h}',h);
}

$('#clear-text').onclick=async()=>{ await browser.runtime.sendMessage({type:'clear',target:'text'}); await refresh(); };
$('#clear-clip').onclick=async()=>{ await browser.runtime.sendMessage({type:'clear',target:'clip'}); await refresh(); };
...
wnBtn.onclick=()=>{ overlay.hidden=false; };
document.addEventListener('click', (e)=>{ if(e.target===overlay || e.target===wnClose) overlay.hidden=true; });
