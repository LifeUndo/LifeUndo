...
// "Что нового" — модалка (безопасный обработчик)
function toggleWn(show){
  const m = document.getElementById('wnModal');
  if(!m) return;
  m.classList.toggle('hidden', !show);
  if (show) document.addEventListener('keydown', onWnEsc);
  else document.removeEventListener('keydown', onWnEsc);
}
function onWnEsc(e){ if (e.key === 'Escape') toggleWn(false); }

// Делегирование кликов (не зависит от конкретных id кнопок)
document.addEventListener('click', (e)=>{
  if (e.target.closest('[data-wn-open]')) { e.preventDefault(); toggleWn(true); }
  if (e.target.closest('[data-wn-close]')) { e.preventDefault(); toggleWn(false); }
  if (e.target.matches('.wn-backdrop')) { toggleWn(false); }
}, true);
