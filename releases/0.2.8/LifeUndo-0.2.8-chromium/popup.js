...
// VIP Import - железобетонная логика
...
function derToRaw(der) {
  // простая DER-парсилка для ECDSA (r,s)
  let i = 0;
...
async function verifyLicenseSignatureFull(obj, embeddedPubJwk) {
  if (!obj?.license || !obj?.signature?.sig_b64) throw new Error('Invalid license object');
  const jwk = obj.signature.publicKeyJwk || embeddedPubJwk;
  if (!jwk) throw new Error('Public key missing');
  const key = await importJwk(jwk);

  const data = new TextEncoder().encode(canonicalStringify(obj.license));
  const sigBytes = b64ToBytes(obj.signature.sig_b64);

  // WebCrypto в разных средах принимает либо RAW, либо DER. Пробуем оба.
  const candidates = [sigBytes];
  try { candidates.push(derToRaw(sigBytes)); } catch (_) {} // если это уже RAW — дер разбор упадёт, игнорим
...
async function doVipImport(file) {
  ...
  try {
    ...
    // Встроенный публичный ключ как запасной вариант
    const embedded = (await import('./licensePubKey.js')).LICENSE_PUB_KEY_JWK;
    const ok = await verifyLicenseSignatureFull(obj, embedded);
