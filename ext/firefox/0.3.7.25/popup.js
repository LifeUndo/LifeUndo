...
async function setInitialLocale() {
  try {
    const storedLocale = await browser.storage.local.get('uiLocale');
    if (storedLocale.uiLocale) {
      currentLocale = storedLocale.uiLocale;
    } else {
      const browserLang = navigator.language.toLowerCase();
      currentLocale = browserLang.startsWith('ru') ? 'ru' : 'en';
      await browser.storage.local.set({ uiLocale: currentLocale });
    }
...
function createListItem(text, origin, timestamp) {
  ...
  sub.textContent = origin + ' • ' + new Date(timestamp).toLocaleString();
...
function fillList(listId, items, clickHandler) {
  ...
  items.forEach(item => {
    const listItem = createListItem(item.text, item.origin, item.ts);
