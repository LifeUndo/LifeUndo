...
// Получение текущей локали UI
async function getUILocale() {
  try {
    const { lu_ui_locale } = await browser.storage.local.get('lu_ui_locale');
    if (lu_ui_locale) return lu_ui_locale;
  } catch (error) {
    console.error('Error getting UI locale:', error);
  }
  ...
  try {
    await browser.storage.local.set({ lu_ui_locale: locale });
  } catch (error) {
    console.error('Error saving UI locale:', error);
  }
...
function applyI18n(locale) {
  ...
  if (currentLangSpan) {
    currentLangSpan.textContent = locale.toUpperCase();
  }
}
...
function initLangSwitcher() {
  ...
  langOptions.forEach(option => {
    option.addEventListener('click', async (e) => {
      ...
      try {
        await browser.storage.local.set({ lu_ui_locale: selectedLang });
        applyI18n(selectedLang);
        updateLinks(selectedLang);
...
// Обновление ссылок
async function updateLinks(locale) {
...
document.addEventListener('DOMContentLoaded', async () => {
  // Получаем локаль и применяем локализацию
  ...
  // Обновляем ссылки
  await updateLinks(locale);
  
  // Проверяем защищённую страницу
