...
console.log('🔥 Building Firefox extension 0.3.7.14...');
...
try {
  ...
  const xpiFile = path.join(projectRoot, 'release', 'amo', 'lifeundo-0.3.7.14.xpi');
  const changelogFile = path.join(projectRoot, 'release', 'amo', 'CHANGELOG-0.3.7.14.txt');
  ...
  if (fs.existsSync(xpiFile)) {
    ...
    let checksumContent = '';
    checksumContent += `${xpiHash} *lifeundo-0.3.7.14.xpi\n`;
    ...
    if (fs.existsSync(changelogFile)) {
      ...
      checksumContent += `${changelogHash} *CHANGELOG-0.3.7.14.txt\n`;
    }
...
console.log('📁 XPI location: release/amo/lifeundo-0.3.7.14.xpi');
